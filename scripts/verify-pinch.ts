...
async function run() {
  ...
  const before = await page.evaluate(() => ({ scale: window.__LOTUS_STUDY__?.scene?.camera?.zoom, theta: window.__LOTUS_STUDY__?.scene?.interactiveCameraScript?.theta }));
  ...
  const after = await page.evaluate(() => ({ scale: window.__LOTUS_STUDY__?.scene?.camera?.zoom, theta: window.__LOTUS_STUDY__?.scene?.interactiveCameraScript?.theta }));
